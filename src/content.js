void (async () => {
  const { Temporal } = await import("node_modules/@js-temporal/polyfill");
  ...
  function parseTimeEntry(element) {
    return {
      start: Temporal.PlainTime(
        element.querySelector(".entry-timestamp-start").textContent
      ),
      end: Temporal.PlainTime(
        element.querySelector(".entry-timestamp-end").textContent
      ),
    };
  }
